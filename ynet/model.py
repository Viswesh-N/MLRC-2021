...
class YNetEncoder(pl.LightningModule):
...
class YNetDecoder(pl.LightningModule):
...
class YNetTorch(pl.LightningModule):
	...
	def train(self, train_data, val_data, params, train_image_path, val_image_path, experiment_name, batch_size=8, num_goals=20, num_traj=1, device=None, dataset_name=None):
		...
		for e in tqdm(range(params['num_epochs']), desc='Epoch'):
			...
			if val_ADE < best_test_ADE:
				...
				wandb.save_model_wandb(self.save_path)
				print(f"Saved model to: {self.save_path}")
			wandb.log_losses(loss=train_loss, mode="train", epoch=e)
			wandb.log_metrics(metrics =metric_dict, mode="test", epoch=e)
			wandb.log_summary(best_metrics=best_metrics)
